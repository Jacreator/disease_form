...
// Static data
    ...
    private FormLayout buildForm() {
        ...
        ageYears = new TextField();
        ageYears.setPlaceholder("Estimated Years");
        ageYears.setTooltipText("Estimated Years");

        ageMonths = new TextField();
        ageMonths.setPlaceholder("Estimated Months");
        ageMonths.setTooltipText("Estimated Months");
        ageMonths.setEnabled(false); // Disabled as in original

        HorizontalLayout ageLayout = new HorizontalLayout(ageYears, ageMonths);
        ageLayout.setWidthFull();
        ageLayout.setSpacing(true);
        ageYears.setWidthFull();
        ageMonths.setWidthFull();

        // Create a wrapper for age with label
        VerticalLayout ageWrapper = new VerticalLayout();
        ageWrapper.setPadding(false);
        ageWrapper.setSpacing(false);
        Span ageLabel = new Span("Age");
        ageLabel.getStyle().set("font-weight", "bold");
        ageLabel.getStyle().set("font-size", "var(--lumo-font-size-s)");
        ageLabel.getStyle().set("color", "var(--lumo-secondary-text-color)");
        ageWrapper.add(ageLabel, ageLayout);
